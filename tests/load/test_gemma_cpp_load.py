...
async def run_load_test(tracer, target_model: str, load_test_config):
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
        print(f"Warming up Local AI Router (Gemma) for model '{target_model}'...")
...
def test_gemma_cpp_load(load_test_config):
    """
    Executes a load test against the Gemma 3 12B router.
    ...
    with tracer.start_as_current_span("local_router_load_test.total_execution") as parent_span:
        ...
        print(f"\nStarting INSTRUMENTED ASYNC load test for Gemma model '{target_model}': {total_requests} requests...")
