# Origami AI Router: Composite Model Assessment
...
Mistral was incredibly robust at adhering to JSON generation without API crashes. However, it suffered the highest cognitive drift in the matrix (66%), heavily over-generalizing nuanced questions to the broader `retail_therapy_bot` or `brainiac_supreme` endpoints rather than selecting specialized agents (such as `deets_detective`).
...
Currently, the distinction between agents like `rule_book_nerd` and `chaos_coordinator` relies on implicit cloud-tier reasoning. 
* **The Fix:** Inject explicit negative constraints into the Agent `description` properties inside `rules.toml`.
* *Example:* "Use `chaos_coordinator` exclusively for active order issues. DO NOT use this for generalized company policies—route those to `rule_book_nerd`."

### B. Defining Stateful Fallback Rules
Cloud models intrinsically suspect that "Add this to my cart" is not a routing agent command. Edge models blindly attempt to route it to `retail_therapy_bot`, failing the test. 
