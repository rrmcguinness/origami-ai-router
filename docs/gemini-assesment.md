...
The model cleanly recognized that actionable commands attempting to directly modify external system state (e.g., "add this to my cart", "add to my essentials list") are outside the bounds of stateless routing, correctly and consistently redirecting those intents to the `fallback` orchestrator for session-based cart management.
